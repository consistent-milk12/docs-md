...
Therefore, many programs that want to manipulate paths *do* assume they contain UTF-8 data, and convert them to [`str`](#str)s
...
the path in place. It also implements `Deref` to [`Utf8Path`](#utf8path), meaning that
...
- <span id="utf8pathbuf-new"></span>`const fn new() -> Utf8PathBuf` — [`Utf8PathBuf`](#utf8pathbuf)
...
- <span id="utf8pathbuf-from-path-buf"></span>`fn from_path_buf(path: PathBuf) -> Result<Utf8PathBuf, PathBuf>` — [`Utf8PathBuf`](#utf8pathbuf)
  ...
  Creates a new [`Utf8PathBuf`](#utf8pathbuf) from a [`PathBuf`](#pathbuf) containing valid UTF-8 characters.
  
  Errors with the original [`PathBuf`](#pathbuf) if it is not valid UTF-8.
...
- <span id="utf8pathbuf-from-os-string"></span>`fn from_os_string(os_string: OsString) -> Result<Utf8PathBuf, OsString>` — [`Utf8PathBuf`](#utf8pathbuf)
  ...
  Creates a new [`Utf8PathBuf`](#utf8pathbuf) from an [`OsString`](#osstring) containing valid UTF-8 characters.
  
  Errors with the original [`OsString`](#osstring) if it is not valid UTF-8.
...
- <span id="utf8pathbuf-into-std-path-buf"></span>`fn into_std_path_buf(self) -> PathBuf`
  ...
  Converts a [`Utf8PathBuf`](#utf8pathbuf) to a [`PathBuf`](#pathbuf).
...
- <span id="utf8pathbuf-add-extension"></span>`fn add_extension<S: AsRef<str>>(&mut self, extension: S) -> bool`
...
- <span id="utf8pathbuf-try-reserve-exact"></span>`fn try_reserve_exact(&mut self, additional: usize) -> Result<(), std::collections::TryReserveError>`
  ...
  Invokes `try_reserve_exact` on the underlying instance of [`PathBuf`](#pathbuf).
  
  *Requires Rust 1.63 or newer.*

- <span id="utf8pathbuf-shrink-to-fit"></span>`fn shrink_to_fit(&mut self)`

  Invokes `shrink_to_fit` on the underlying instance of [`PathBuf`](#pathbuf).

- <span id="utf8pathbuf-shrink-to"></span>`fn shrink_to(&mut self, min_capacity: usize)`

  Invokes `shrink_to` on the underlying instance of [`PathBuf`](#pathbuf).
...
- <span id="utf8path-new"></span>`fn new(s: &impl AsRef<str> + ?Sized) -> &Utf8Path` — [`Utf8Path`](#utf8path)
  ...
  You can create [`Utf8Path`](#utf8path)s from [`String`](#string)s, or even other [`Utf8Path`](#utf8path)s:
...
- <span id="utf8path-from-path"></span>`fn from_path(path: &Path) -> Option<&Utf8Path>` — [`Utf8Path`](#utf8path)
  ...
  Converts a `Path` to a [`Utf8Path`](#utf8path).
...
- <span id="utf8path-from-os-str"></span>`fn from_os_str(path: &OsStr) -> Option<&Utf8Path>` — [`Utf8Path`](#utf8path)
  ...
  Converts an `OsStr` to a [`Utf8Path`](#utf8path).
...
- <span id="utf8path-as-std-path"></span>`fn as_std_path(&self) -> &Path`
  ...
  Converts a [`Utf8Path`](#utf8path) to a `Path`.
...
- <span id="utf8path-as-str"></span>`fn as_str(&self) -> &str`
  ...
  Yields the underlying [`str`](#str) slice.
...
- <span id="utf8path-file-prefix"></span>`fn file_prefix(&self) -> Option<&str>`
...
- <span id="utf8path-join-os"></span>`fn join_os(&self, path: impl AsRef<Path>) -> PathBuf`
  ...
  Creates an owned [`PathBuf`](#pathbuf) with `path` adjoined to `self`.
...
- <span id="utf8path-with-added-extension"></span>`fn with_added_extension<S: AsRef<str>>(&self, extension: S) -> Utf8PathBuf` — [`Utf8PathBuf`](#utf8pathbuf)

  Creates an owned [`Utf8PathBuf`](#utf8pathbuf) like `self` but with the extension added.
...
- <span id="utf8path-iter"></span>`fn iter(&self) -> Iter<'_>` — [`Iter`](#iter)
  ...
  Produces an iterator over the path's components viewed as [`str`](#str)
  slices.
...
- <span id="utf8path-canonicalize"></span>`fn canonicalize(&self) -> io::Result<PathBuf>`
  ...
  This returns a [`PathBuf`](#pathbuf) because even if a symlink is valid Unicode, its target may not
...
- <span id="utf8path-into-path-buf"></span>`fn into_path_buf(self: Box<Utf8Path>) -> Utf8PathBuf` — [`Utf8Path`](#utf8path), [`Utf8PathBuf`](#utf8pathbuf)
  ...
  Converts a `Box<Utf8Path>` into a [`Utf8PathBuf`](#utf8pathbuf) without copying or allocating.
...
[`Utf8PrefixComponent`](#utf8prefixcomponent) also holds the raw and unparsed [`str`](#str) slice,
...
- <span id="utf8prefixcomponent-as-str"></span>`fn as_str(&self) -> &'a str`
  ...
  Returns the [`str`](#str) slice for this prefix.
...
- <span id="utf8direntry-into-inner"></span>`fn into_inner(self) -> fs::DirEntry`
  ...
  Returns the original `fs::DirEntry` within this [`Utf8DirEntry`](#utf8direntry).
...
- <span id="frompathbuferror-as-path"></span>`fn as_path(&self) -> &Path`
  ...
  Returns the `Path` slice that was attempted to be converted to [`Utf8PathBuf`](#utf8pathbuf).

- <span id="frompathbuferror-into-path-buf"></span>`fn into_path_buf(self) -> PathBuf`

  Returns the [`PathBuf`](#pathbuf) that was attempted to be converted to [`Utf8PathBuf`](#utf8pathbuf).
...
- <span id="frompathbuferror-into-io-error"></span>`fn into_io_error(self) -> io::Error`
  ...
  Many users of [`FromPathBufError`](#frompathbuferror) will want to convert it into an `io::Error`. This is a
...
- <span id="frompatherror-into-io-error"></span>`fn into_io_error(self) -> io::Error`
  ...
  Many users of [`FromPathError`](#frompatherror) will want to convert it into an `io::Error`. This is a
  convenience method to do that.
...
- <span id="fromosstringerror-as-os-str"></span>`fn as_os_str(&self) -> &OsStr`
  ...
  Returns the `OsStr` slice that was attempted to be converted to [`Utf8PathBuf`](#utf8pathbuf).

- <span id="fromosstringerror-into-os-string"></span>`fn into_os_string(self) -> OsString`

  Returns the [`OsString`](#osstring) that was attempted to be converted to [`Utf8PathBuf`](#utf8pathbuf).
...
- <span id="fromosstringerror-into-io-error"></span>`fn into_io_error(self) -> io::Error`
  ...
  Many users of [`FromOsStringError`](#fromosstringerror) will want to convert it into an `io::Error`.
...
- <span id="fromosstrerror-into-io-error"></span>`fn into_io_error(self) -> io::Error`
  ...
  Many users of [`FromOsStrError`](#fromosstrerror) will want to convert it into an `io::Error`. This is a
  convenience method to do that.
