...
There are two methods for doing so: [`with_default`](subscriber/index.md) and
[`set_global_default`](subscriber/index.md). `with_default` sets the default subscriber for the
...
```
Then, we can use [`with_default`](subscriber/index.md) to set our `Dispatch` as the default for
...
As an alternative to `with_default`, we can use [`set_global_default`](subscriber/index.md) to
...
See [`strip_str`](../../anstream/index.md)
