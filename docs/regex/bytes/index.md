...
Search for regex matches in `&[u8](#u8)` haystacks.
...
1. Matching is done on `&[u8](#u8)` instead of `&str`. Additionally, `Vec<u8>`
...
// valid UTF-8.
let (_, [title](#title)) = re.captures(hay).unwrap().extract();
...
In general, one should expect performance on `&[u8](#u8)` to be roughly similar to
performance on `&str`.
