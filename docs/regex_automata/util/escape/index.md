...
Since this crate tends to deal with `&[u8](#u8)` everywhere and the default
`Debug` implementation just shows decimal integers, it makes debugging those
representations quite difficult. This module provides types that show `&[u8](#u8)`
...
Provides a convenient `Debug` implementation for `&[u8](#u8)`.
