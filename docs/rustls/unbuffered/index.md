...
This is an alternative to the [`crate::ConnectionCommon`](#connectioncommon) API that does not internally buffer
...
user. Unlike the `ConnectionCommon` API, this API does not make use of the [`std::io::Read`](#read) and
[`std::io::Write`](#write) traits so it's usable in no-std context.

The entry points into this API are [`crate::client::UnbufferedClientConnection::new`](#new),
[`crate::server::UnbufferedServerConnection::new`](#new) and
[`unbuffered::UnbufferedConnectionCommon::process_tls_records`](#process-tls-records). The state machine API is
documented in [`unbuffered::ConnectionState`](#connectionstate).
...
- **`discard`**: `usize`
  ...
  This number MUST be added to the value of [`UnbufferedStatus::discard`](#discard) *prior* to the
  discard operation. See [`UnbufferedStatus::discard`](#discard) for more details
...
- **`discard`**: `usize`
  ...
  the [`UnbufferedConnectionCommon::process_tls_records`](#process-tls-records) call that returned this object.
  
  This discard operation MUST happen *before*
  [`UnbufferedConnectionCommon::process_tls_records`](#process-tls-records) is called again.
...
- **`state`**: `Result<ConnectionState<'c, 'i, Data>, crate::Error>`
  ...
  [`UnbufferedConnectionCommon::process_tls_records`](#process-tls-records) again. See the documentation on the
  variants of [`ConnectionState`](#connectionstate) for more details.
...
The state of the [`UnbufferedConnectionCommon`](#unbufferedconnectioncommon) object
...
- **`ReadTraffic`**
  ...
  See [`ReadTraffic`](#readtraffic) for more details on how to use the enclosed object to access
  the received data.
...
- **`PeerClosed`**
  ...
  [`WriteTraffic::queue_close_notify()`](#queue-close-notify).
...
- **`EncodeTlsData`**
  ...
  Call [`EncodeTlsData::encode`](#encode) on the enclosed object, providing an `outgoing_tls`
...
- **`TransmitTlsData`**
  ...
  [`EncodeTlsData::encode`](#encode) calls to the peer.
  
  After transmitting the contents, call [`TransmitTlsData::done`](#done) on the enclosed object.
  ...
  handshake records. Call [`TransmitTlsData::may_encrypt_app_data`](#may-encrypt-app-data) on the enclosed
...
- **`BlockedHandshake`**
  ...
  was passed to [`UnbufferedConnectionCommon::process_tls_records`](#process-tls-records).

- **`WriteTraffic`**

  The handshake process has been completed.
  
  [`WriteTraffic::encrypt`](#encrypt) can be called on the enclosed object to encrypt application
  data into an `outgoing_tls` buffer. Similarly, [`WriteTraffic::queue_close_notify`](#queue-close-notify) can
  ...
  [`UnbufferedConnectionCommon::process_tls_records`](#process-tls-records) invocation. When enough data has been
  appended to `incoming_tls`, [`UnbufferedConnectionCommon::process_tls_records`](#process-tls-records) will yield
  the [`ConnectionState::ReadTraffic`](#readtraffic) state.
