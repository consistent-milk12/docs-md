*[tracing_core](../index.md) / [span](index.md)*

---

# Module `span`

Spans represent periods of time in the execution of a program.

## Structs

### `Id`

```rust
struct Id(core::num::NonZeroU64);
```

Identifies a span within the context of a subscriber.

They are generated by [`Subscriber`](../subscriber/index.md)s for each span as it is created, by
the `new_span` trait method. See the documentation for that method for
more information on span ID generation.



#### Implementations

- `fn from_u64(u: u64) -> Self`

- `const fn from_non_zero_u64(id: NonZeroU64) -> Self`

- `fn into_u64(self: &Self) -> u64`

- `const fn into_non_zero_u64(self: &Self) -> NonZeroU64`

#### Trait Implementations

##### `impl Clone for Id`

- `fn clone(self: &Self) -> Id` — [`Id`](#id)

##### `impl Debug for Id`

- `fn fmt(self: &Self, f: &mut $crate::fmt::Formatter<'_>) -> $crate::fmt::Result`

##### `impl Eq for Id`

##### `impl Hash for Id`

- `fn hash<__H: $crate::hash::Hasher>(self: &Self, state: &mut __H)`

##### `impl PartialEq for Id`

- `fn eq(self: &Self, other: &Id) -> bool` — [`Id`](#id)

##### `impl StructuralPartialEq for Id`

### `Attributes<'a>`

```rust
struct Attributes<'a> {
    metadata: &'static crate::Metadata<'static>,
    values: &'a field::ValueSet<'a>,
    parent: crate::parent::Parent,
}
```

Attributes provided to a `Subscriber` describing a new span when it is
created.

#### Implementations

- `fn new(metadata: &'static Metadata<'static>, values: &'a field::ValueSet<'a>) -> Self` — [`Metadata`](../metadata/index.md), [`ValueSet`](../field/index.md)

- `fn new_root(metadata: &'static Metadata<'static>, values: &'a field::ValueSet<'a>) -> Self` — [`Metadata`](../metadata/index.md), [`ValueSet`](../field/index.md)

- `fn child_of(parent: Id, metadata: &'static Metadata<'static>, values: &'a field::ValueSet<'a>) -> Self` — [`Id`](#id), [`Metadata`](../metadata/index.md), [`ValueSet`](../field/index.md)

- `fn metadata(self: &Self) -> &'static Metadata<'static>` — [`Metadata`](../metadata/index.md)

- `fn values(self: &Self) -> &field::ValueSet<'a>` — [`ValueSet`](../field/index.md)

- `fn is_root(self: &Self) -> bool`

- `fn is_contextual(self: &Self) -> bool`

- `fn parent(self: &Self) -> Option<&Id>` — [`Id`](#id)

- `fn record(self: &Self, visitor: &mut dyn field::Visit)` — [`Visit`](../field/index.md)

- `fn contains(self: &Self, field: &field::Field) -> bool` — [`Field`](../field/index.md)

- `fn is_empty(self: &Self) -> bool`

- `fn fields(self: &Self) -> &FieldSet` — [`FieldSet`](../field/index.md)

#### Trait Implementations

##### `impl<'a> Debug for Attributes<'a>`

- `fn fmt(self: &Self, f: &mut $crate::fmt::Formatter<'_>) -> $crate::fmt::Result`

##### `impl RecordFields for tracing_core::span::Attributes<'_>`

- `fn clone_span(self: &Self, id: &span::Id) -> span::Id` — [`Id`](#id)

### `Record<'a>`

```rust
struct Record<'a> {
    values: &'a field::ValueSet<'a>,
}
```

A set of fields recorded by a span.

#### Implementations

- `fn new(values: &'a field::ValueSet<'a>) -> Self` — [`ValueSet`](../field/index.md)

- `fn record(self: &Self, visitor: &mut dyn field::Visit)` — [`Visit`](../field/index.md)

- `fn len(self: &Self) -> usize`

- `fn contains(self: &Self, field: &field::Field) -> bool` — [`Field`](../field/index.md)

- `fn is_empty(self: &Self) -> bool`

#### Trait Implementations

##### `impl<'a> Debug for Record<'a>`

- `fn fmt(self: &Self, f: &mut $crate::fmt::Formatter<'_>) -> $crate::fmt::Result`

##### `impl RecordFields for tracing_core::span::Record<'_>`

- `fn drop_span(self: &Self, _id: span::Id)` — [`Id`](#id)

### `Current`

```rust
struct Current {
    inner: CurrentInner,
}
```

Indicates what [the `Subscriber` considers] the "current" span.

As subscribers may not track a notion of a current span, this has three
possible states:
- "unknown", indicating that the subscriber does not track a current span,
- "none", indicating that the current context is known to not be in a span,
- "some", with the current span's [`Id`](#id) and [`Metadata`](../metadata/index.md).



#### Implementations

- `fn new(id: Id, metadata: &'static Metadata<'static>) -> Self` — [`Id`](#id), [`Metadata`](../metadata/index.md)

- `fn none() -> Self`

- `fn unknown() -> Self`

- `fn is_known(self: &Self) -> bool`

- `fn into_inner(self: Self) -> Option<(Id, &'static Metadata<'static>)>` — [`Id`](#id), [`Metadata`](../metadata/index.md)

- `fn id(self: &Self) -> Option<&Id>` — [`Id`](#id)

- `fn metadata(self: &Self) -> Option<&'static Metadata<'static>>` — [`Metadata`](../metadata/index.md)

#### Trait Implementations

##### `impl Debug for Current`

- `fn fmt(self: &Self, f: &mut $crate::fmt::Formatter<'_>) -> $crate::fmt::Result`

