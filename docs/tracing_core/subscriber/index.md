...
A no-op [`Subscriber`](tracing_core/subscriber/index.md).

[`NoSubscriber`](tracing_core/subscriber/index.md) implements the [`Subscriber`](tracing_core/subscriber/index.md) trait by never being enabled,
...
- [`register_callsite`](#register-callsite) is called once for each callsite from which a span
  event may originate, and returns an [`Interest`](tracing_core/subscriber/index.md) value describing whether or
  not the subscriber wishes to see events or spans from that callsite. By
  default, it calls [`enabled`](../../tracing/tracing/index.md), and returns `Interest::always()` if
...
- [`event_enabled`](../../tracing/tracing/index.md) is called once before every call to the [`event`](tracing_core/event/index.md)
...
- `fn on_register_dispatch(self: &Self, subscriber: &Dispatch)`
  ...
  Invoked when this subscriber becomes a [`Dispatch`](tracing_core/dispatcher/index.md).
...
- `fn event_enabled(self: &Self, event: &Event<'_>) -> bool`
  ...
  Determine if an [`Event`](tracing_core/event/index.md) should be recorded.

- `fn event(self: &Self, event: &Event<'_>)`

  Records that an [`Event`](tracing_core/event/index.md) has occurred.
