...
Every span or event in `tracing` is associated with a [`Callsite`](tracing_core/callsite/index.md). A
callsite is a small `static` value that is responsible for the following:

* Storing the span or event's [`Metadata`](tracing_core/metadata/index.md),
...
[`register`](tracing_core/callsite/index.md)s itself with the global callsite registry. Registering a
callsite calls the [`Subscriber::register_callsite`][`register_callsite`](#register-callsite)
method with that callsite's [`Metadata`](tracing_core/metadata/index.md) on every currently active
...
If a [`Subscriber`](tracing_core/subscriber/index.md) implementation wishes to allocate storage for each
...
The [`register_callsite`](#register-callsite) method returns an [`Interest`](tracing_core/subscriber/index.md) value,
...
When registering a new callsite, the [`Interest`](tracing_core/subscriber/index.md)s returned by every
...
future, the cached [`Interest`](tracing_core/subscriber/index.md) value can be checked efficiently
to determine if the span or event should be recorded, without
needing to perform expensive filtering (i.e. calling the
[`Subscriber::enabled`](#enabled) method every time a span or event occurs).

### Rebuilding Cached Interest

When a new [`Dispatch`](tracing_core/dispatcher/index.md) is created (i.e. a new subscriber becomes
active), any previously cached [`Interest`](tracing_core/subscriber/index.md) values are re-evaluated
...
[`Interest`](tracing_core/subscriber/index.md) in that callsite is updated. Similarly, when a
...
In addition, the [`rebuild_interest_cache`](tracing_core/callsite/index.md) function in this module can be
...
[`register_callsite`](#register-callsite) method, so that its [`Subscriber::enabled`](#enabled) method
...
[`rebuild_interest_cache`](tracing_core/callsite/index.md) to re-evaluate the entire interest cache with its
...
[`Subscriber::enabled`](#enabled) frequently.

# Implementing Callsites

In most cases, instrumenting code using `tracing` should *not* require
implementing the [`Callsite`](tracing_core/callsite/index.md) trait directly. When using the [`tracing`
crate's macros][macros](#macros) or the [`#[instrument](#instrument)` attribute][instrument](#instrument), a
...
events, the [`DefaultCallsite`](tracing_core/callsite/index.md) struct provided in this module is a
...
[`Callsite`](tracing_core/callsite/index.md) for user types, as `DefaultCallsite` may benefit from
...
Uniquely identifies a [`Callsite`](tracing_core/callsite/index.md)
...
A default [`Callsite`](tracing_core/callsite/index.md) implementation.
...
- `fn set_interest(self: &Self, interest: Interest)`
  ...
  Sets the [`Interest`](tracing_core/subscriber/index.md) for this callsite.
...
Clear and reregister interest on every [`Callsite`](tracing_core/callsite/index.md)

This function is intended for runtime reconfiguration of filters on traces
when the filter recalculation is much less frequent than trace events are.
The alternative is to have the [`Subscriber`](tracing_core/subscriber/index.md) that supports runtime
reconfiguration of filters always return [`Interest::sometimes()`](#sometimes) so that
[`enabled`](../../tracing/tracing/index.md) is evaluated for every event.

This function will also re-compute the global maximum level as determined by
the [`max_level_hint`](#max-level-hint) method. If a [`Subscriber`](tracing_core/subscriber/index.md)
...
Register a new [`Callsite`](tracing_core/callsite/index.md) with the global registry.
